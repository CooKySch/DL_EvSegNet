...
if __name__ == "__main__":
    ...
    parser.add_argument("--percentage_data_used", help="portion of the dataset used, e.g. 0.5 is 50%", default=0.1)
    parser.add_argument("--log_dir", help="where the tensorboard logs are stored", default='logs/')
    parser.add_argument("--hyperparam", help="perform hyperparameter tuning? True/False", default=1)
    ...
    if hyperparam_tuning == 1:
      ...
      batch_range = np.array([16])
      lr_range = np.array([0.1, 0.05, 0.01, 0.005, 0.0001, 0.00005, 0.00001, 0.000005, 0.000001])
      ...
      for index_lr, learning_rate in tqdm(enumerate(lr_range)):
          ...
          for index_batch, batch_size in enumerate(batch_range):
              ...
              np.save("/home/bram/DL_EvSegNet/test_accs_batch{}_lr{}.npy".format(batch_size, learning_rate), np.array(test_accs))
              np.save("/home/bram/DL_EvSegNet/mious_batch{}_lr{}.npy".format(batch_size, learning_rate), np.array(mious))


      # plot results
      np.save("/home/bram/DL_EvSegNet/test_accs.npy", np.array(test_accs))
      np.save("/home/bram/DL_EvSegNet/mious.npy", np.array(mious))
